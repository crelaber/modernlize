...
public class HtmlCompressor {
	...
	private static Pattern prePattern = Pattern.compile("<pre[^>]*?>.*?</pre>",
			Pattern.DOTALL | Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
	private static Pattern taPattern = Pattern.compile(
			"<textarea[^>]*?>.*?</textarea>", Pattern.DOTALL
					| Pattern.CASE_INSENSITIVE | Pattern.MULTILINE);
	private static Pattern jspPattern = Pattern.compile(
			"<%([^-@][\\w\\W]*?)%>", Pattern.DOTALL | Pattern.CASE_INSENSITIVE
					| Pattern.MULTILINE);
	// <script></script>
	...
	// 字符串匹配
	private static Pattern stringPattern = Pattern
			.compile("(\"[^\"\\n]*?\"|'[^'\\n]*?')");
	// trim去空格和换行符
	...
	private static String processJspBlocks(String html, List<String> blocks) {
		...
		while (result.contains(tempJspBlock)) {
			result = result.replaceFirst(tempJspBlock,
					Matcher.quoteReplacement(blocks.remove(0)));
		}
	...
	private static String processPreBlocks(String html, List<String> blocks)
		...
		while (result.contains(tempPreBlock)) {
			result = result.replaceFirst(tempPreBlock,
					Matcher.quoteReplacement(blocks.remove(0)));
		}
	...
	private static String processTextareaBlocks(String html, List<String> blocks)
		...
		while (result.contains(tempTextAreaBlock)) {
			result = result.replaceFirst(tempTextAreaBlock,
					Matcher.quoteReplacement(blocks.remove(0)));
		}
	...
	private static String processScriptBlocks(String html, List<String> blocks)
		...
		while (result.contains(tempScriptBlock)) {
			result = result.replaceFirst(tempScriptBlock,
					Matcher.quoteReplacement(blocks.remove(0)));
		}
	...
	private static String processStyleBlocks(String html, List<String> blocks)
		...
		while (result.contains(tempStyleBlock)) {
			result = result.replaceFirst(tempStyleBlock,
					Matcher.quoteReplacement(blocks.remove(0)));
		}
	...
	private static String compressJsp(String source) {
		...
		if (jspMatcher.find()) {
			...
			return (new StringBuilder(source.substring(0, jspMatcher.start(1)))
					.append(result).append(source.substring(jspMatcher.end(1))))
					.toString();
		} else {
			return source;
		}
	...
	private static String compressJavaScript(String source) {
		...
		if (scriptMatcher.find()) {
			...
			return (new StringBuilder(source.substring(0,
					scriptMatcher.start(1))).append(result).append(source
	...
	private static String compressCssStyles(String source) {
		...
		if (styleMatcher.find()) {
			// 去掉注释，换行
			String result = multiCommentPattern.matcher(styleMatcher.group(1))
					.replaceAll("");
			result = trimPattern.matcher(result).replaceAll("");
			result = trimPattern2.matcher(result).replaceAll("");
			return (new StringBuilder(
					source.substring(0, styleMatcher.start(1))).append(result)
		...
		while (stringMatcher.find()) {
			...
			if (tmpStr.indexOf("//") != -1 || tmpStr.indexOf("/*") != -1
				...
				String blockStr = tmpStr
						.replaceAll("//", tempSingleCommentBlock)
