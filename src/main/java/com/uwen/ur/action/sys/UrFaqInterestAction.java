...
@Controller
@Scope("prototype")
public class UrFaqInterestAction extends BaseAction{
    /**
     * 
     */
    private Logger log = LoggerFactory.getLogger(UrFaqInterestAction.class);
    /**
     * 服务
     */
    @Autowired
    protected UrFaqInterestService service;
     ...
     * pojo
     */
    protected UrFaqInterest record;
        ...
        try {
     		if (JUtil.isNULL(record.getId())) {
                if (1 == dbFlag) {
                    ...
                    service.insertUrFaqInterest(record);
                }
            } else {
		        record.setUpdateDatetime(new Date());
                if (1 == dbFlag) {
                    // 0 全部
                    service.modifyUrFaqInterestByPk(record);
                } else {
                    //选择性的(判断 isNull)
                    service.modifyUrFaqInterestByPkSelective(record);
                }
            }
        } catch (Exception e) {
    ...
    public String findByPk() {
        try {
            ...
            UrFaqInterest rec = new UrFaqInterest();
            rec.setId(record.getId());
            JSONObject obj = service.findUrFaqInterestAndFkText(rec, new MapperCallBackAdapter(), appendArray);
            set$data(obj, prefix);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            set$data(new BasicJSON(0,"fail",null));
    ...
    public String find() {
        try {
            FieldAppendArray appendArray = new FieldAppendArray();
            JSONObject obj = service.findUrFaqInterestAndFkText(record, null, appendArray);
            set$data(obj, prefix);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            set$data(new BasicJSON(0,"fail",null));
    ...
    public String listByPk() {
        try {
            ...
            JSONArray array = service.listUrFaqInterestAndFkText(rec, sort, paging, new MapperCallBackAdapter(){
        ...
        try {
            ...
            JSONArray array = service.listUrFaqInterestAndFkText(rec, sort, paging, new MapperCallBackAdapter(){
                ...
                public void doCriteria(Object obj) throws Exception{
                    MapperUtil.builderCriteria(obj, UrFaqInterest.class, beanProxy);
                }
            }, appendArray);
            //返回数据
            if(comp == 1){
    ...
    public String delete(){
        try {
            ...
            UrFaqInterest delRecord = new UrFaqInterest();
            delRecord.setId(record.getId());
            delRecord.setEnable(0);
            service.modifyUrFaqInterestByPkSelective(delRecord);
    ...
    public String drop(){
        try {
            ...
            UrFaqInterest delRecord = new UrFaqInterest();
            delRecord.setId(record.getId());
            service.dropUrFaqInterest(delRecord, new MapperCallBackAdapter());
            //pageData.put("state", "success");
            //pageData.put("msg", "操作成功!");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    public UrFaqInterest getRecord() {
        return record;
    }

    public void setRecord(UrFaqInterest record) {
        this.record = record;
    }
