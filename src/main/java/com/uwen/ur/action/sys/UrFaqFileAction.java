...
@Controller
@Scope("prototype")
public class UrFaqFileAction extends BaseAction{
    /**
     * 
     */
    private Logger log = LoggerFactory.getLogger(UrFaqFileAction.class);
    /**
     * 服务
     */
    @Autowired
    protected UrFaqFileService service;
        ...
        try {
     		if (JUtil.isNULL(record.getId())) {
                if (1 == dbFlag) {
                    ...
                    service.insertUrFaqFile(record);
                }
            } else {
		        record.setUpdateDatetime(new Date());
                if (1 == dbFlag) {
                    // 0 全部
                    service.modifyUrFaqFileByPk(record);
                } else {
                    //选择性的(判断 isNull)
                    service.modifyUrFaqFileByPkSelective(record);
                }
            }
        } catch (Exception e) {
    ...
    public String findByPk() {
        try {
            ...
            UrFaqFile rec = new UrFaqFile();
            rec.setId(record.getId());
            JSONObject obj = service.findUrFaqFileAndFkText(rec, new MapperCallBackAdapter(), appendArray);
            set$data(obj, prefix);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            set$data(new BasicJSON(0,"fail",null));
    ...
    public String find() {
        try {
            FieldAppendArray appendArray = new FieldAppendArray();
            JSONObject obj = service.findUrFaqFileAndFkText(record, null, appendArray);
            set$data(obj, prefix);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            set$data(new BasicJSON(0,"fail",null));
    ...
    public String listByPk() {
        try {
            ...
            JSONArray array = service.listUrFaqFileAndFkText(rec, sort, paging, new MapperCallBackAdapter(){
        ...
        try {
            ...
            JSONArray array = service.listUrFaqFileAndFkText(rec, sort, paging, new MapperCallBackAdapter(){
                ...
                public void doCriteria(Object obj) throws Exception{
                    MapperUtil.builderCriteria(obj, UrFaqFile.class, beanProxy);
                }
            }, appendArray);
            //返回数据
            if(comp == 1){
    ...
    public String delete(){
        try {
            ...
            UrFaqFile delRecord = new UrFaqFile();
            delRecord.setId(record.getId());
            delRecord.setEnable(0);
            service.modifyUrFaqFileByPkSelective(delRecord);
    ...
    public String drop(){
        try {
            ...
            UrFaqFile delRecord = new UrFaqFile();
            delRecord.setId(record.getId());
            service.dropUrFaqFile(delRecord, new MapperCallBackAdapter());
            //pageData.put("state", "success");
            //pageData.put("msg", "操作成功!");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    public UrFaqFile getRecord() {
        return record;
    }

    public void setRecord(UrFaqFile record) {
        this.record = record;
    }
