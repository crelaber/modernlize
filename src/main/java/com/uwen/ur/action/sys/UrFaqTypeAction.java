...
@Controller
@Scope("prototype")
public class UrFaqTypeAction extends BaseAction{
    /**
     * 
     */
    private Logger log = LoggerFactory.getLogger(UrFaqTypeAction.class);
    /**
     * 服务
     */
    @Autowired
    protected UrFaqTypeService service;
        ...
        try {
     		if (JUtil.isNULL(record.getId())) {
                if (1 == dbFlag) {
                    ...
                    service.insertUrFaqType(record);
                }
            } else {
		        record.setUpdateDatetime(new Date());
                if (1 == dbFlag) {
                    // 0 全部
                    service.modifyUrFaqTypeByPk(record);
                } else {
                    //选择性的(判断 isNull)
                    service.modifyUrFaqTypeByPkSelective(record);
                }
            }
        } catch (Exception e) {
    ...
    public String findByPk() {
        try {
            ...
            JSONObject obj = service.findUrFaqTypeAndFkText(rec, new MapperCallBackAdapter(), appendArray);
            set$data(obj, prefix);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            set$data(new BasicJSON(0,"fail",null));
    ...
    public String find() {
        try {
            FieldAppendArray appendArray = new FieldAppendArray();
            JSONObject obj = service.findUrFaqTypeAndFkText(record, null, appendArray);
            set$data(obj, prefix);
        } catch (Exception e) {
            log.error(e.getMessage(), e);
            set$data(new BasicJSON(0,"fail",null));
    ...
    public String listByPk() {
        try {
            ...
            JSONArray array = service.listUrFaqTypeAndFkText(rec, sort, paging, new MapperCallBackAdapter(){
    ...
    public String delete(){
        try {
            ...
            UrFaqType delRecord = new UrFaqType();
            delRecord.setId(record.getId());
            delRecord.setEnable(0);
            service.modifyUrFaqTypeByPkSelective(delRecord);
    ...
    public String drop(){
        try {
            ...
            UrFaqType delRecord = new UrFaqType();
            delRecord.setId(record.getId());
            service.dropUrFaqType(delRecord, new MapperCallBackAdapter());
            //pageData.put("state", "success");
            //pageData.put("msg", "操作成功!");
        } catch (Exception e) {
            log.error(e.getMessage(), e);
    ...
    public UrFaqType getRecord() {
        return record;
    }

    public void setRecord(UrFaqType record) {
        this.record = record;
    }
